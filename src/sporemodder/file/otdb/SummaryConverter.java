...
public class SummaryConverter implements Converter {
	...
	private static final int TYPE_ID = 0x02D5C9AF;
	...
	public boolean decode(StreamReader stream, File outputFolder, ResourceKey key) throws Exception {
		return decode(stream, Converter.getOutputFile(key, outputFolder, "summary_t"));
	}

	@Override
	public boolean encode(File input, StreamWriter output) throws Exception {
		SummaryFile file = new SummaryFile();
		ArgScriptStream<SummaryFile> stream = file.generateStream();
	...
	public boolean isEncoder(File file) {
		checkExtensions();
		return file.isFile() && file.getName().endsWith("." + extension + ".summary_t");
	}

	@Override
	public String getName() {
		return "Summary File (." + HashManager.get().getTypeName(TYPE_ID) + ")";
	}
