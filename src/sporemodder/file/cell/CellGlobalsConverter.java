...
public class CellGlobalsConverter implements Converter {
	
	private static final int TYPE_ID = 0x2A3CE5B7;
	...
	public boolean decode(StreamReader stream, File outputFolder, ResourceKey key) throws Exception {
		return decode(stream, Converter.getOutputFile(key, outputFolder, "globals_t"));
	}
	...
	public boolean isEncoder(File file) {
		checkExtensions();
		return file.isFile() && file.getName().endsWith("." + extension + ".globals_t");
	}

	@Override
	public String getName() {
		return "Cell Stage Globals File (." + HashManager.get().getTypeName(TYPE_ID) + ")";
	}
