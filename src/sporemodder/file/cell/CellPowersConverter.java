...
public class CellPowersConverter implements Converter {
	
	private static final int TYPE_ID = 0x754BE343;
	private static String extension = null;
	
	private boolean decode(StreamReader stream, File outputFile) throws Exception {
		CellPowersFile unit = new CellPowersFile();
		unit.read(stream);
	...
	public boolean decode(StreamReader stream, File outputFolder, ResourceKey key) throws Exception {
		return decode(stream, Converter.getOutputFile(key, outputFolder, "powers_t"));
	}

	@Override
	public boolean encode(File input, StreamWriter output) throws Exception {
		CellPowersFile unit = new CellPowersFile();
		ArgScriptStream<CellPowersFile> stream = unit.generateStream();
	...
	public boolean isEncoder(File file) {
		checkExtensions();
		return file.isFile() && file.getName().endsWith("." + extension + ".powers_t");
	}

	@Override
	public String getName() {
		return "Cell Stage Powers File (." + HashManager.get().getTypeName(TYPE_ID) + ")";
	}
