...
public class CellRandomCreatureConverter implements Converter {
	
	private static final int TYPE_ID = 0xF9C3D770;
	private static String extension = null;
	
	private boolean decode(StreamReader stream, File outputFile) throws Exception {
		CellRandomCreatureFile unit = new CellRandomCreatureFile();
		unit.read(stream);
	...
	public boolean decode(StreamReader stream, File outputFolder, ResourceKey key) throws Exception {
		return decode(stream, Converter.getOutputFile(key, outputFolder, "random_creature_t"));
	}

	@Override
	public boolean encode(File input, StreamWriter output) throws Exception {
		CellRandomCreatureFile unit = new CellRandomCreatureFile();
		ArgScriptStream<CellRandomCreatureFile> stream = unit.generateStream();
	...
	public boolean isEncoder(File file) {
		checkExtensions();
		return file.isFile() && file.getName().endsWith("." + extension + ".random_creature_t");
	}

	@Override
	public String getName() {
		return "Cell Stage Random Creature File (." + HashManager.get().getTypeName(TYPE_ID) + ")";
	}
