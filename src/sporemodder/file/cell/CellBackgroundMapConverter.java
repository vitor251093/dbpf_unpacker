...
public class CellBackgroundMapConverter implements Converter {
	
	private static final int TYPE_ID = 0x612B3191;
	private static String extension = null;
	
	private boolean decode(StreamReader stream, File outputFile) throws Exception {
		CellBackgroundMapFile unit = new CellBackgroundMapFile();
		unit.read(stream);
	...
	public boolean decode(StreamReader stream, File outputFolder, ResourceKey key) throws Exception {
		return decode(stream, Converter.getOutputFile(key, outputFolder, "backgroundMap_t"));
	}

	@Override
	public boolean encode(File input, StreamWriter output) throws Exception {
		CellBackgroundMapFile unit = new CellBackgroundMapFile();
		ArgScriptStream<CellBackgroundMapFile> stream = unit.generateStream();
	...
	public boolean isEncoder(File file) {
		checkExtensions();
		return file.isFile() && file.getName().endsWith("." + extension + ".backgroundMap_t");
	}

	@Override
	public String getName() {
		return "Cell Stage Background Map File (." + HashManager.get().getTypeName(TYPE_ID) + ")";
	}
