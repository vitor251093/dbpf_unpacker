...
public class CellLookTableConverter implements Converter {
	
	private static final int TYPE_ID = 0x8C042499;
	...
	public boolean decode(StreamReader stream, File outputFolder, ResourceKey key) throws Exception {
		return decode(stream, Converter.getOutputFile(key, outputFolder, "look_table_t"));
	}
	...
	public boolean isEncoder(File file) {
		checkExtensions();
		return file.isFile() && file.getName().endsWith("." + extension + ".look_table_t");
	}

	@Override
	public String getName() {
		return "Cell Stage Look Table File (." + HashManager.get().getTypeName(TYPE_ID) + ")";
	}
