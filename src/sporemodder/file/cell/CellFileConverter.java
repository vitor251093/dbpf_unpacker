...
public class CellFileConverter implements Converter {
	
	private static final int TYPE_ID = 0xDFAD9F51;
	...
	public boolean decode(StreamReader stream, File outputFolder, ResourceKey key) throws Exception {
		return decode(stream, Converter.getOutputFile(key, outputFolder, "cell_t"));
	}
	...
	public boolean isEncoder(File file) {
		checkExtensions();
		return file.isFile() && file.getName().endsWith("." + extension + ".cell_t");
	}

	@Override
	public String getName() {
		return "Cell Stage Cell File (." + HashManager.get().getTypeName(TYPE_ID) + ")";
	}
