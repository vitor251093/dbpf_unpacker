...
public class CellEffectMapConverter implements Converter {
	
	private static final int TYPE_ID = 0x433FB70C;
	private static String extension = null;
	
	private boolean decode(StreamReader stream, File outputFile) throws Exception {
		CellEffectMapFile unit = new CellEffectMapFile();
		unit.read(stream);
	...
	public boolean decode(StreamReader stream, File outputFolder, ResourceKey key) throws Exception {
		return decode(stream, Converter.getOutputFile(key, outputFolder, "effectMap_t"));
	}

	@Override
	public boolean encode(File input, StreamWriter output) throws Exception {
		CellEffectMapFile unit = new CellEffectMapFile();
		ArgScriptStream<CellEffectMapFile> stream = unit.generateStream();
	...
	public boolean isEncoder(File file) {
		checkExtensions();
		return file.isFile() && file.getName().endsWith("." + extension + ".effectMap_t");
	}

	@Override
	public String getName() {
		return "Cell Stage Effect Map File (." + HashManager.get().getTypeName(TYPE_ID) + ")";
	}
